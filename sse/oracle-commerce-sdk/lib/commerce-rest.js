...
const VALID_JSON_CONTENT_TYPES = ['application/json', 'application/json;charset=utf-8','application/json;charset=UTF-8', 'application/json; charset=utf-8','application/json; charset=UTF-8'];
...
CommerceSDK.prototype.login = function () {
  ...
  var loginPromise = new Promise (
    ...
    function (resolve, reject) {
      ...
      self.request ({url : '/ccadmin/v1/login',
        method : 'post',
...
CommerceSDK.prototype.logout = function () {
  ...
  var logoutPromise = new Promise (
    function (resolve, reject) {
      ...
      self.post ({url : '/ccadmin/v1/logout', data : {},  callback : function (err, response) {
          ...
          if (response.result === true) {
            ...
            CommerceSDK.printDebugMessage('user logged out and the accessToken value is ::' + self.accessToken, self.logger);
            resolve (self);
          }
        }});//end of post
...
CommerceSDK.prototype.isValidToken = function () {
  ...
  var validPromise = new Promise (
    function (resolve, reject) {
      ...
      self.request ({url : '/ccadmin/v1/verify', method: 'post', data : requestData, callback : function (err, response) {
...
CommerceSDK.prototype.request = function (args) {
  ...
  if (requestMethod === 'post' || requestMethod === 'put' || requestMethod === 'delete') {
    ...
    CommerceSDK.printDebugMessage ('The content type values is :: ' + requestOptions.headers['Content-Type'], self.logger);
    CommerceSDK.printDebugMessage ('The content length values is :: ' + requestOptions.headers['Content-Length'], self.logger);
  }
    ...
    function (resolve, reject) {
      ...
      if (!self.accessToken) {
        ...
        tokenPromise.then (function (success) {
          ...
          CommerceSDK.printError ('There was an error while trying to initialize and trying to fix it ' , err, self.logger);
          let loginPromise = self.login();
          loginPromise.then (function (lp) {// jshint ignore:line
            resolve (true);
          }).catch(function (err) {
